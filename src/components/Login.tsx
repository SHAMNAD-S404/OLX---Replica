...
function Login(props:popupProp) {

  const { setUser } = useAuth() //its for set user state
  const navigate = useNavigate() // to redirect to diff urls
  ...
  const googleSingin = async () => {
    try {
      ...
      setUser(result.user.displayName) // set user details in context
      props?.setLoginPop(false) //to hide login modal
          ...
          <div className="sm:flex sm:items-start">
            ...
            <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">             
              <div className="mt-2">
                ...
                <div onClick= {googleSingin} className='flex border border-gray-300 p-2 rounded-md mt-6' >
